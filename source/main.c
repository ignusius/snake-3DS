...
void update(State *statep)
{
    ...
    if ((kDown & KEY_START) )restart(statep);
    else if ((kDown & KEY_UP) && statep->direction != DOWN)
        statep->direction = UP;
    else if ((kDown & KEY_DOWN) && statep->direction != UP)
        statep->direction = DOWN;
    else if ((kDown & KEY_LEFT) && statep->direction != RIGHT)
        statep->direction = LEFT;
    else if ((kDown & KEY_RIGHT) && statep->direction != LEFT)
        statep->direction = RIGHT;
    hidScanInput();

    if (!statep->crashed)
    {
         if (!statedraw) moveSnake(statep);
...
void init(State *statep)
{
    ...
    uint32_t flags = SDL_HWSURFACE | SDL_CONSOLEBOTTOM;
...
int main(int argc, char **argv)
{
    State state;
    init(&state);
    printf("\x1b[16;11H\"START\" - new game");
